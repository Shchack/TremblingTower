...
namespace PluginMaster
{
...
#if !PWB_3_4_OR_NEWER
            if (dataVersion.IsOlderThan("3.4"))
            {
                void ImportShortcut(string shortcutId, PWBKeyShortcut PWBShortcut)
                {
                    var shortcut = UnityEditor.ShortcutManagement.ShortcutManager.instance.GetShortcutBinding(shortcutId);
                    var keyCombinationSecuence = shortcut.keyCombinationSequence.ToArray();
                    if (keyCombinationSecuence.Length == 0) return;
                    var keyCombination = keyCombinationSecuence[0];
                    var modifiers = keyCombination.modifiers;
                    var eventModifiers = EventModifiers.None;
                    if ((modifiers & UnityEditor.ShortcutManagement.ShortcutModifiers.Alt)
                        == UnityEditor.ShortcutManagement.ShortcutModifiers.Alt) eventModifiers |= EventModifiers.Alt;
                    if ((modifiers & UnityEditor.ShortcutManagement.ShortcutModifiers.Action)
                        == UnityEditor.ShortcutManagement.ShortcutModifiers.Action) eventModifiers |= EventModifiers.Control;
                    if ((modifiers & UnityEditor.ShortcutManagement.ShortcutModifiers.Shift)
                        == UnityEditor.ShortcutManagement.ShortcutModifiers.Shift) eventModifiers |= EventModifiers.Shift;
                    PWBShortcut.combination.Set(keyCombination.keyCode, eventModifiers);
                    PWBSettings.UpdateShrotcutsConflictsAndSaveFile();
                }
                ImportShortcut(Shortcuts.PWB_TOGGLE_PIN_SHORTCUT_ID, PWBSettings.shortcuts.toolbarPinToggle);
                ImportShortcut(Shortcuts.PWB_TOGGLE_BRUSH_SHORTCUT_ID, PWBSettings.shortcuts.toolbarBrushToggle);
                ImportShortcut(Shortcuts.PWB_TOGGLE_GRAVITY_SHORTCUT_ID, PWBSettings.shortcuts.toolbarGravityToggle);
                ImportShortcut(Shortcuts.PWB_TOGGLE_LINE_SHORTCUT_ID, PWBSettings.shortcuts.toolbarLineToggle);
                ImportShortcut(Shortcuts.PWB_TOGGLE_SHAPE_SHORTCUT_ID, PWBSettings.shortcuts.toolbarShapeToggle);
                ImportShortcut(Shortcuts.PWB_TOGGLE_TILING_SHORTCUT_ID, PWBSettings.shortcuts.toolbarTilingToggle);
                ImportShortcut(Shortcuts.PWB_TOGGLE_REPLACER_SHORTCUT_ID, PWBSettings.shortcuts.toolbarReplacerToggle);
                ImportShortcut(Shortcuts.PWB_TOGGLE_ERASER_SHORTCUT_ID, PWBSettings.shortcuts.toolbarEraserToggle);
                ImportShortcut(Shortcuts.PWB_TOGGLE_SELECTION_SHORTCUT_ID, PWBSettings.shortcuts.toolbarSelectionToggle);
                ImportShortcut(Shortcuts.PWB_TOGGLE_EXTRUDE_SHORTCUT_ID, PWBSettings.shortcuts.toolbarExtrudeToggle);
                ImportShortcut(Shortcuts.PWB_TOGGLE_MIRROR_SHORTCUT_ID, PWBSettings.shortcuts.toolbarMirrorToggle);
                const string DEF_SYMBOL = "PWB_3_4_OR_NEWER";
                ...
                if (!definesSCSV.Contains(DEF_SYMBOL))
                {
                    if (!string.IsNullOrEmpty(definesSCSV)) definesSCSV += ";";
                    definesSCSV += DEF_SYMBOL;
    ...
    public class PWBShortcuts
    {
        ...
        private PWBKeyShortcut _toolbarPinToggle = new PWBKeyShortcut("Toggle Pin Tool",
            PWBShortcut.Group.GLOBAL, KeyCode.Alpha1, EventModifiers.Alt | EventModifiers.Shift);
        [SerializeField]
        private PWBKeyShortcut _toolbarBrushToggle = new PWBKeyShortcut("Toggle Brush Tool",
            PWBShortcut.Group.GLOBAL, KeyCode.Alpha2, EventModifiers.Alt | EventModifiers.Shift);
        [SerializeField]
        private PWBKeyShortcut _toolbarGravityToggle = new PWBKeyShortcut("Toggle Gravity Tool",
            PWBShortcut.Group.GLOBAL, KeyCode.Alpha3, EventModifiers.Alt | EventModifiers.Shift);
        [SerializeField]
        private PWBKeyShortcut _toolbarLineToggle = new PWBKeyShortcut("Toggle Line Tool",
            PWBShortcut.Group.GLOBAL, KeyCode.Alpha4, EventModifiers.Alt | EventModifiers.Shift);
        [SerializeField]
        private PWBKeyShortcut _toolbarShapeToggle = new PWBKeyShortcut("Toggle Shape Tool",
            PWBShortcut.Group.GLOBAL, KeyCode.Alpha5, EventModifiers.Alt | EventModifiers.Shift);
        [SerializeField]
        private PWBKeyShortcut _toolbarTilingToggle = new PWBKeyShortcut("Toggle Tiling Tool",
            PWBShortcut.Group.GLOBAL, KeyCode.Alpha6, EventModifiers.Alt | EventModifiers.Shift);
        [SerializeField]
        private PWBKeyShortcut _toolbarReplacerToggle = new PWBKeyShortcut("Toggle Replacer Tool",
            PWBShortcut.Group.GLOBAL, KeyCode.Alpha7, EventModifiers.Alt | EventModifiers.Shift);
        [SerializeField]
        private PWBKeyShortcut _toolbarEraserToggle = new PWBKeyShortcut("Toggle Eraser Tool",
            PWBShortcut.Group.GLOBAL, KeyCode.Alpha8, EventModifiers.Alt | EventModifiers.Shift);
        [SerializeField]
        private PWBKeyShortcut _toolbarSelectionToggle = new PWBKeyShortcut("Toggle Selection Tool",
            PWBShortcut.Group.GLOBAL, KeyCode.Alpha9, EventModifiers.Alt | EventModifiers.Shift);
        [SerializeField]
        private PWBKeyShortcut _toolbarExtrudeToggle = new PWBKeyShortcut("Toggle Extrude Tool",
            PWBShortcut.Group.GLOBAL, KeyCode.X, EventModifiers.Alt | EventModifiers.Shift);
        [SerializeField]
        private PWBKeyShortcut _toolbarMirrorToggle = new PWBKeyShortcut("Toggle Mirror Tool",
            PWBShortcut.Group.GLOBAL, KeyCode.M, EventModifiers.Alt | EventModifiers.Shift);
